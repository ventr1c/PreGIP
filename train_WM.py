...
#%%
n_cluster = 5
idx_pair = gcl.get_reference(dataloader,number=n_cluster)
...
for _ in range(args.num_wm):
    ...
    key_normal.append(dataset[idx_pair[c]])
...
for seed in range(10,10+args.num_repeat):
    ...
    acc_fix, IP_fix = gcl.test(trainloader,testloader,key_normal, key_watermark,args)
    acc_finetune, IP_finetune = gcl.test_finetune(trainloader,testloader,key_normal, key_watermark,args)
    ...
    acc_finetune, IP_finetune = ind_gcl.test_finetune(trainloader,testloader,key_normal, key_watermark,args)
