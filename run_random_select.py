...
parser.add_argument('--watermark_size', type=int, default=1,
                    help='Number of hidden water marks.')
parser.add_argument('--wm_num_node', type=int, default=200,
                    help='Number of nodes in a watermark graph.')
...
parser.add_argument('--normal_selection', type=str, default='distance')
...
if(args.normal_selection == 'random'):
    idx_pair = utils.obtain_idx_random(args.seed, np.arange(len(dataset)), args.num_wm)
elif(args.normal_selection in ['distance', 'cluster']):
    gcl_shadow = copy.deepcopy(gcl)
    idx_pair = utils.obtain_idx_distance(args, np.arange(len(dataset)), args.num_wm, gcl_shadow, dataset, device)
...
split_train = utils.get_split_self(num_samples=len(dataset), train_ratio=0.5, test_ratio=0.4,device=device)
