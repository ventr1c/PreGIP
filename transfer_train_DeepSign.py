...
from models.DeepSign import Encoder
...
pretrain_pretrain_loader = DataLoader(pretrain_pretrain_dataset, batch_size=args.batch_size,shuffle=True)
if args.alpha == 0.0:
    if(args.if_load_watermark):
        print("Loading watermarked networks")
        gcl.encoder.load_state_dict(torch.load("./checkpoint/{}_GCL/deepsign_wm_gcl_{}.pt".format('ZINC200k',args.alpha)))
    else:
<<<<<<< HEAD
        gcl.pretrain(pretrain_pretrain_loader, args,verbose=args.debug)
=======
        gcl.pretrain(pretrain_pretrain_dataset, args,verbose=args.debug)
>>>>>>> b99538ad237ca2f0286619ea31de9e82c9204f15
        torch.save(gcl.encoder.state_dict(), "./checkpoint/{}_GCL/deepsign_wm_gcl_{}.pt".format('ZINC200k',args.alpha))
else:   
    if(args.if_load_watermark):
        print("Loading watermarked networks")
        gcl.encoder.load_state_dict(torch.load("./checkpoint/{}_GCL/deepsign_wm_gcl_{}.pt".format('ZINC200k',args.alpha)))
