...
parser.add_argument('--watermark_size', type=int, default=5,
                    help='size of hidden water marks.')
...
from models.WPE import PairData
wm_generator = WatermarkGraph(args, device)
wm_graph = wm_generator.gene_random_graph(dataset.data.x,  args.watermark_size, p = 0.5, feat_generation = 'Rand_Gene')
wm_graph = Data(x=wm_graph[0],edge_index=wm_graph[1])
pair_dataset = []
for idx in range(len(dataset)):
    trigger = inject_trigger(0,dataset[idx],wm_graph)
    pair_data = PairData(x_s=dataset[idx].x, edge_index_s = dataset[idx].edge_index,\
                        x_t=trigger.x, edge_index_t =trigger.edge_index)
    pair_dataset.append(pair_data)
...
#%%
from models.WPE import Encoder
gnn = GIN(dataset.num_features, args.num_hidden, num_layers=2).to(device)
pair_loader = DataLoader(pair_dataset,batch_size=args.batch_size,shuffle=True, follow_batch=['x_s', 'x_t'])
wpe = Encoder(gnn,device).to(device)
wpe.watermarking(gcl,pair_loader,args,verbose=args.debug)
...
for idx in idx_pair:
    ...
    pair_data = PairData(x_s=dataset[idx].x, edge_index_s = dataset[idx].edge_index,\
                        x_t=trigger.x, edge_index_t =trigger.edge_index)
    test_watermark.append(pair_data)
test_watermark = Batch.from_data_list(test_watermark,follow_batch=['x_s', 'x_t'])
...
for seed in range(10,10+args.num_repeat):
    ...
    acc_fix, IP_fix = wpe.test(trainloader,testloader, test_watermark,args)
    acc_finetune, IP_finetune = wpe.test_finetune(trainloader,testloader, test_watermark,args)
    ...
    ind_gcl = Encoder(encoder=ind_gnn,device=device).to(device)
