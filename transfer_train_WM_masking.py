...
#%%
from models.WM_Masking import Encoder
...
# gcl.encoder.load_state_dict(torch.load("checkpoint/GCL/gcl_10.pt"))
gcl.encoder.load_state_dict(torch.load("./checkpoint/{}_Masking/mask_{}.pt".format('ZINC200k', 10)))

n_cluster = 5
'''sample only 200k graphs from ZINC'''
sampled_idx2 = utils.obtain_idx_random(args.seed, np.array(list(range(len(pretrain_dataset)))), size = int(len(pretrain_dataset) * 0.1))
sampled_pretrain_dataset = pretrain_dataset[sampled_idx2]
sampled_pretrain_dataloader = DataLoader(sampled_pretrain_dataset, batch_size=args.batch_size,shuffle=True)
idx_pair = gcl.get_reference(sampled_pretrain_dataloader,number=n_cluster)
...
for _ in range(args.num_wm):
    c = np.random.randint(n_cluster)
    key_normal.append(sampled_pretrain_dataset[idx_pair[c]])
    ...
    watermark_feat, watermark_edge_index, watermark_edge_weight, watermark_edge_attr = \
                    wm_generator.gene_random_graph(pretrain_dataset.data.x, pretrain_dataset.data.edge_attr, num_nodes, p = 0.2, feat_generation = 'Rand_Samp', attr_generation = 'Rand_Samp')
#     watermark_feat, watermark_edge_index, watermark_edge_weight = \
#                     wm_generator.WMRandom(dataset.data.x, num_nodes, p = 0.2)
    key_watermark.append(Data(x=watermark_feat,edge_index=watermark_edge_index, edge_attr=watermark_edge_attr))

key_normal = Batch.from_data_list(key_normal)
key_watermark = Batch.from_data_list(key_watermark)


# gcl.watermarking(key_normal, key_watermark, pretrain_dataloader, args, verbose=args.debug)
if(args.if_load_watermark):
    print("Loading watermarked networks")
    gcl.encoder.load_state_dict(torch.load("./checkpoint/{}_GCL/CertWM_wm_masking_{}_{}_{}.pt".format('ZINC200k',args.alpha, args.watermark_size, args.random)))
else:
<<<<<<< HEAD
    gcl.watermarking(key_normal, key_watermark, pretrain_dataloader, args, verbose=args.debug)
=======
    gcl.watermarking(key_normal, key_watermark, pretrain_dataset, args, verbose=args.debug)
>>>>>>> b99538ad237ca2f0286619ea31de9e82c9204f15
    torch.save(gcl.encoder.state_dict(), "./checkpoint/{}_GCL/CertWM_wm_masking_{}_{}_{}.pt".format('ZINC200k',args.alpha, args.watermark_size, args.random))
#%%
acc_fix_list = []
acc_finetune_list = []
...
for seed in range(10,10+args.num_repeat):
    ...
    '''Evaluation'''
<<<<<<< HEAD
    acc_fix, IP_fix = wm_gcl.test(trainloader,testloader, key_normal, key_watermark,args)
    acc_finetune, IP_finetune = wm_gcl.test_finetune(trainloader,testloader, key_normal, key_watermark,num_tasks,args)
=======
...
<<<<<<< HEAD
    acc_fix, IP_fix = ind_gcl.test(trainloader,testloader, key_normal, key_watermark,args)
    acc_finetune, IP_finetune = ind_gcl.test_finetune(trainloader,testloader, key_normal, key_watermark,num_tasks,args)
=======
...
# nohup bash transfer_train_WM.sh > ./logs/transfer_noncertiWM_graphcl_tox21.log 2>&1 &
# nohup bash transfer_train_WM_Certified.sh > ./logs/transfer_certiWM_graphcl_tox21.log 2>&1 &