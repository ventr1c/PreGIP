    ...
    def watermarking(self, key_normal, key_watermark, dataloader, args,verbose=True):
        ...
        for epoch in range(args.num_epochs):
            ...
            for data in dataloader:
                ...
                loss = pred_loss + args.alpha *meta_loss
            ...
            epoch_acc_edge = acc_edge_accum/len(dataloader)
            if verbose and epoch % 5 == 0:
                print('Epoch {}, training loss: {:.6f}, node acc: {:.4f}, edge acc: {:.4f}, Watermark loss: {:.6f}, Meta IP loss: {:.6f}'.format(epoch,epoch_loss,epoch_acc_node,epoch_acc_edge, wm_loss.item(), meta_loss.item()))
