...
class GAT_Conv(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, heads=8, dropout=0.2,
                 encoder_norm='batch', projector_norm='batch'):
        ...
        self.layers = torch.nn.ModuleList()
        self.layers.append(GATConv(input_dim, hidden_dim,heads,dropout=dropout))
        for _ in range(1, num_layers - 1):
            self.layers.append(GATConv(heads*hidden_dim,heads*hidden_dim,dropout=dropout))
        
        self.layers.append(GATConv(heads*hidden_dim,hidden_dim,concat=False, dropout=dropout))
...
class BGRL(torch.nn.Module):
    def __init__(self, args, nfeat, nhid, nproj, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,tau=None, layer=2,if_smoothed=True,device=None):
        ...
        if(args.base_model == 'GCN'):
            ...
            self.gconv = GAT_Conv(input_dim=nfeat, hidden_dim=nhid, num_layers=layer, heads = 8, dropout= 0.1).to(device)
    ...
    def test(self,x, edge_index, edge_weight, y,idx_train, idx_test, idx_val):
        return test(self.encoder_model,x, edge_index, edge_weight, y,idx_train, idx_test, idx_val)
    ...
    def aug_forward(self, x, edge_index, batch=None, edge_weight=None):
        x1, edge_index1, edge_weight1 = self.aug1(x, edge_index, edge_weight)
        h1, h2, _, _, _, _ = self.encoder_model(x1, edge_index1,edge_weight1)
        z = torch.cat([h1, h2], dim=1)
        return z
...
def test(encoder_model, x, edge_index, edge_weight, y,idx_train, idx_test, idx_val):
    encoder_model.eval()
    ...
    # print(split)
    split = {}
    split['train'], split['test'], split['valid'] = idx_train, idx_test, idx_val
