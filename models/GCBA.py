    ...
    def test(self, model, dataset, idx_train, idx_clean_test, idx_atk):
        ...
        with torch.no_grad():
            for data in clean_dataloader:
                ...
                _, g = model.encoder(data.x, data.edge_index, data.batch, edge_weight=data.edge_weight)
        ...
        for idx in idx_atk:
            ...
            bkd_x, bkd_edge_index, bkd_edge_weight = self.inject_trigger(idx_attach, data.x, data.edge_index, data.edge_weight, self.device)
        ...
        bkd_dataloader = DataLoader(dataset, batch_size=self.args.batch_size, shuffle=False)
        with torch.no_grad():
        ...
        for _ in range(1000):
            optimizer.zero_grad()
            pred = fc(x_atk[idx_atk])
            loss = torch.nn.functional.cross_entropy(pred,y_atk[idx_atk])
            loss.backward()
            optimizer.step()

        pred_atk = torch.softmax(fc(x_atk[idx_atk]),dim=1)
        atk_asr = (pred_atk.argmax(dim=1)==self.args.target_class).float().mean()
        atk_asr_no_true = (pred_atk.argmax(dim=1)!=y_atk[idx_atk]).float().mean()
