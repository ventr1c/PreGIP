...
class BGRL_G2L(torch.nn.Module):
    ...
    def test(self, dataloader, idx_labeled_train, idx_test, idx_labeled_val):
        return test(self.encoder_model, dataloader, idx_labeled_train, idx_test, idx_labeled_val)
    ...
    def aug_forward(self, x, edge_index, batch=None, edge_weight=None):
        ...
        h1, h2, _, _, _, _ = self.encoder_model(x1, edge_index1,edge_weight1,batch=batch)
        z = torch.cat([h1, h2], dim=1)
        return z
...
def test(encoder_model, dataloader,idx_train, idx_test, idx_val):
    encoder_model.eval()
    x = []
    y = []
    for data in dataloader:
        data = data.to(encoder_model.device)
        if data.x is None:
            num_nodes = data.batch.size(0)
...
def test_SVM(encoder_model, dataloader):
    encoder_model.eval()
    x = []
    y = []
    for data in dataloader:
