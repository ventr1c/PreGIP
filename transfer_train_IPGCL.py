...
for idx in idx_pair:
    ...
    watermark_feat, watermark_edge_index, watermark_edge_weight, watermark_edge_attr = \
                    wm_generator.gene_random_graph(pretrain_dataset.data.x, pretrain_dataset.data.edge_attr, num_nodes, p = p, feat_generation = 'Rand_Samp', attr_generation = 'Rand_Samp')
...
if(args.if_load_watermark):
    ...
    gcl.encoder.load_state_dict(torch.load("./checkpoint/{}_GCL/wm_gcl_{}_{}_{}.pt".format('ZINC200k',args.step, args.alpha, args.watermark_size)))
else:
    # gcl.pretrain(dataloader, args,verbose=args.debug)
    gcl.watermarking(key_syn, key_watermark, pretrain_dataloader, args, verbose=args.debug)
    torch.save(gcl.encoder.state_dict(), "./checkpoint/{}_GCL/wm_gcl_{}_{}_{}.pt".format('ZINC200k',args.step, args.alpha, args.watermark_size))
